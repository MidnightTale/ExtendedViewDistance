...
import club.tesseract.extendedviewdistance.api.branch.*;
...
import club.tesseract.extendedviewdistance.core.util.SchedulerUtil;
...
//    );
    ...
    while (canRun.get()) {
      ...
      if (!this.globalPause) {
        try {
          ...
          List<PlayerChunkView> viewList = Arrays.asList(this.playersViewMap.values().toArray(new PlayerChunkView[0]));
          ...
          handleServer:{
            for (World world : worldList) {
              ...
              if (serverNetworkTraffic.exceed(configData.getServerSendTickMaxBytes())) break handleServer;
              ...
              handleWorld:{
                ...
                while (!playersFull && effectiveTime >= System.currentTimeMillis()) {
                  ...
                  for (PlayerChunkView view : worldsViews.getOrDefault(world, new ArrayList<>(0))) {
                    if (serverNetworkTraffic.exceed(configData.getServerSendTickMaxBytes())) break handleServer;
                    if (worldNetworkTraffic.exceed(configWorld.getWorldSendTickMaxBytes())) break handleWorld;
                    synchronized (view.networkTraffic) {
                      ...
                      if (view.networkTraffic.exceed(forciblySendSecondMaxBytes != null
                                      ? (int) (forciblySendSecondMaxBytes *configData.playerNetworkSpeedUseDegree) / 20
                                              : configWorld.getPlayerSendTickMaxBytes()
                              )
                      ) continue;
                      if (configData.autoAdaptPlayerNetworkSpeed && view.networkTraffic.exceed(Math.max(1, view.networkSpeed.avg() * 50))) continue;
                    }
                    ...
                    handlePlayer:{
                      ...
                      try {
                        ...
                        try{
                          // Newer version allows for urgent chunk requests
                          boolean urgent = true;
                          Method requestUrgentChunk = World.class.getMethod("getChunkAtAsync", int.class, int.class, boolean.class, boolean.class);
                          chunkFuture = (CompletableFuture<Chunk>) requestUrgentChunk.invoke(world, chunkX, chunkZ, canGenerated, urgent);
                        }catch (NoSuchMethodError e){
                            try {
                                Method requestUrgentChunk = World.class.getMethod("getChunkAtAsync", int.class, int.class, boolean.class);
                                chunkFuture = (CompletableFuture<Chunk>) requestUrgentChunk.invoke(world, chunkX, chunkZ, canGenerated);
                            }catch (NoSuchMethodError er){
                                Method spigotRequestChunk = World.class.getMethod("getChunkAt", int.class, int.class);
                                chunkFuture = CompletableFuture.supplyAsync( () -> {
                                    try {
                                        return (Chunk) spigotRequestChunk.invoke(world, chunkX, chunkZ);
  ...
  private void sendChunk(
    ...
    if (configWorld.preventXray != null && !configWorld.preventXray.isEmpty()) {
      for (Entry<BlockData, BlockData[]> conversionMap : configWorld.preventXray.entrySet()) chunk.replaceAllMaterial(
