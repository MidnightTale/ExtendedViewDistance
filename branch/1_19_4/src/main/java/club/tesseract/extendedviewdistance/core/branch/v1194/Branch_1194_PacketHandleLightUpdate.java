...
public final class Branch_1194_PacketHandleLightUpdate {
    public Branch_1194_PacketHandleLightUpdate() {
    }

    public void write(FriendlyByteBuf serializer, Branch_1194_ChunkLight light, boolean trustEdges) {
    ...
    private static void saveBitSet(byte[][] nibbleArrays, int index, BitSet notEmpty, BitSet isEmpty, List<byte[]> list) {
        ...
        if (nibbleArray != Branch_1194_ChunkLight.EMPTY) {
            if (nibbleArray == null) {
                isEmpty.set(index);
            } else {
                notEmpty.set(index);
