...
public final class Branch_17_ChunkRegionLoader {
    ...
    public static BranchChunk.Status loadStatus(CompoundTag chunkNBT) {
        CompoundTag levelNBT = chunkNBT.getCompound("Level");
        return Branch_1171_Chunk.ofStatus(ChunkStatus.a(levelNBT.getString("Status")));

    }

    public static BranchChunk loadChunk(ServerLevel worldServer, ChunkPos chunkLocation, CompoundTag chunkNBT, boolean integralHeightmap) {
        ChunkGenerator chunkGenerator = worldServer.chunkSource.generator;
        chunkNBT.contains("DataVersion");
...
//            LOGGER.error("Chunk file at {} is in the wrong location; relocating. (Expected {}, got {})", chunkLocation, chunkLocation, locationNBT);
        ...
        for(int index = 0; index < sectionsListNBT.size(); ++index) {
            ...
            if (sectionNBT.hasKeyOfType("Palette", 9) && sectionNBT.hasKeyOfType("BlockStates", 12)) {
                ...
                Branch_1171_Chunk.recalculateBlockCounts(section);
                if (!section.c()) {
        ...
        return new Branch_1171_Chunk(worldServer, chunkAccess);
    }

    public static BranchChunkLight loadLight(ServerLevel serverLevel, CompoundTag chunkNBT) {
        if (chunkNBT.hasKeyOfType("DataVersion", 99)) {
            int dataVersion = chunkNBT.getInt("DataVersion");
            if (!JUST_CORRUPT_IT && dataVersion > CURRENT_DATA_VERSION) {
                (new RuntimeException("Server attempted to load chunk saved with newer version of minecraft! " + dataVersion + " > " + CURRENT_DATA_VERSION)).printStackTrace();
        ...
        boolean                         isLightOn           = levelNBT.get("isLightOn") != null;
        NBTTagList                      sectionsListNBT     = levelNBT.getList("Sections", 10);
        boolean                         hasSkyLight         = serverLevel.getDimensionManager().hasSkyLight();
        Branch_1171_ChunkLight chunkLight          = new Branch_1171_ChunkLight(serverLevel);
    ...
    public static CompoundTag saveChunk(ServerLevel worldserver, ChunkAccess chunkAccess, Branch_1171_ChunkLight chunkLight, List<Runnable> asyncRunnable) {
        int minSection = worldserver.getMinSection() - 1;
        ChunkPos chunkLocation = chunkAccess.getPos();
        CompoundTag chunkNBT = new CompoundTag();
        CompoundTag levelNBT = new CompoundTag();

        chunkNBT.setInt("DataVersion", SharedConstants.getCurrentVersion().getWorldVersion());
