...
public final class Branch_1171_Chunk implements BranchChunk {
    private final ChunkAccess chunkAccess;
    private final ServerLevel serverLevel;


    public Branch_1171_Chunk(ServerLevel worldServer, ChunkAccess chunkAccess) {
    ...
    public BranchNBT toNBT(BranchChunkLight light, List<Runnable> asyncRunnable) {
        return new Branch_1171_NBT(Branch_17_ChunkRegionLoader.saveChunk(serverLevel, chunkAccess, (Branch_1171_ChunkLight) light, asyncRunnable));
    }
    ...
    public org.bukkit.Chunk getChunk() {
        ...
        if (chunk instanceof EmptyLevelChunk)
            return ((EmptyLevelChunk) chunk).bukkitChunk;
        else if (chunk instanceof LevelChunk)
            return ((LevelChunk) chunk).bukkitChunk;
        else if (chunk instanceof ProtoChunk)
            return new LevelChunk(serverLevel, ((ProtoChunk) chunk), v -> {
            }).getBukkitChunk();
        else
            return null;
    }
    ...
    public BlockState getBlockState(int x, int y, int z) {
        ...
        if (indexY >= 0 && indexY < chunkSections.length) {
            ...
            if (chunkSection != null && !chunkSection.isEmpty()) // hasOnlyAir
                return chunkSection.getBlockState(x & 15, y & 15, z & 15);
        }
        return Blocks.AIR.defaultBlockState();
    }
    public void setBlockState(int x, int y, int z, BlockState iBlockData) {
        int indexY = (y >> 4) - chunkAccess.getMinSection();
        LevelChunkSection[] chunkSections = chunkAccess.getSections();
        if (indexY >= 0 && indexY < chunkSections.length) {
            LevelChunkSection chunkSection = chunkSections[indexY];
            if (chunkSection == null)
                chunkSection = chunkSections[indexY] = new LevelChunkSection(indexY, chunkAccess, serverLevel, false);
    ...
    public BlockData getBlockData(int x, int y, int z) {
        BlockState blockData = getBlockState(x, y, z);
    ...
    public void setBlockData(int x, int y, int z, BlockData blockData) {
        ...
        if (iBlockData != null)
            setBlockState(x, y, z, iBlockData);
    }
    ...
    static {
        try {
            field_LevelChunkSection_nonEmptyBlockCount = LevelChunkSection.class.getDeclaredField("f");
    ...
    public static void recalculateBlockCounts(LevelChunkSection section) {
        ...
        try {
            ...
            // 適用於 spigot (不推薦)
            try {
                Method foreachMethod = PalettedContainer.class.getMethod("a", PalettedContainer.CountConsumer.class);
                foreachMethod.invoke(blocks, forEachLocation);
    ...
    public void replaceAllMaterial(BlockData[] target, BlockData to) {
        ...
        for (LevelChunkSection section :chunkAccess.getSections()) {
            if (section != null) {
                ...
                try {
                    Method setBlockStateMethod = PalettedContainer.class.getMethod("b", int.class, int.class, int.class, BlockState.class);
                    conversionLocationList.forEach(location -> {
                        try {
                            setBlockStateMethod.invoke(blocks, location & 15, location >> 8 & 15, location >> 4 & 15, toI);
                        }catch (Exception e) {
                            e.printStackTrace();
                        }
                    });
    ...
    public org.bukkit.block.Biome getBiome(int x, int y, int z) {
        ChunkBiomeContainer biomeStorage = chunkAccess.getBiomes();//.getBiomeIndex();
        return biomeStorage != null ? CraftBlock.biomeBaseToBiome(serverLevel.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY), biomeStorage.getNoiseBiome(x, y, z)) : null;
    }
    ...
    public void setBiome(int x, int y, int z, org.bukkit.block.Biome biome) {
        ChunkBiomeContainer biomeStorage = chunkAccess.getBiomes();
        if (biomeStorage != null)
            biomeStorage.setBiome(x, y, z, CraftBlock.biomeToBiomeBase(serverLevel.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY), biome));
    }

    public boolean hasFluid(int x, int y, int z) {
        return !getBlockState(x, y, z).getFluidState().isEmpty();
    }
    ...
    public int getHighestY(int x, int z) {
        return chunkAccess.getHeight(Heightmap.Types.WORLD_SURFACE, x, z);
    }
